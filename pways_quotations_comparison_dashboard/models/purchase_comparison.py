...
class ResConfigSettings(models.TransientModel):
    ...
    is_remaing_cancel = fields.Boolean(related="company_id.is_remaing_cancel", string="Cancel Non Selected RFQ",
                                       readonly=False)
...
class PurchaseComparison(models.Model):
    ...
    state = fields.Selection(
        [('draft', 'Draft'), ('approve', 'Set To Approve'), ('approved', 'Approved'), ('inprogress', 'In progress'),
    ...
    def get_purchase_line_data(self, option, reqisition_id=None):
        ...
        product_ids = self.env['purchase.order.line'].search([('order_id', 'in', purchase_ids.ids)]).mapped(
            'product_id')
        ...
        for product in product_ids:
            ...
            min_price_order_line_id = order_lines.filtered(
                lambda x: x.price_unit and x.price_unit == min_price_order_line)
            min_date_order_line_id = order_lines.filtered(
                lambda x: x.date_planned and x.date_planned == min_date_order_line)
        ...
        for product_id in product_ids:
            ...
            record.append({
                ...
                'description': self.env['purchase.order.line'].search(
                    [('order_id', 'in', purchase_ids.ids), ('product_id', '=', product_id.id)], limit=1).name,
                'min_prize': min_prize,
    ...
    def confirm_order_action(self, purchase_id=None, reqisition_id=None):
        ...
        if comparison_id.env.company.is_remaing_cancel:
            cancel_purchase_ids = comparison_id.comparison_line_ids.mapped('purchase_id').filtered(
                lambda x: x.id != confirm_purchase_id.id)
        ...
        display_msg = """ RFQ """ + str(confirm_purchase_id.name) + """
                                      ...
                                      with amount of """ + str(
            confirm_purchase_id.amount_total) + """ on date """ + today_date.strftime("%Y-%m-%d")
